...
class ScrapPremiumThread(Thread):
    def __init__(self):
        Thread.__init__(self)
        self.df_roller_pvc37 = pd.DataFrame()
        self.df_roller_pvc52 = pd.DataFrame()
        self.df_roller_alu39 = pd.DataFrame()
        self.df_roller_alu52 = pd.DataFrame()
 
    def run(self):
        time.sleep(1)
        driver = webdriver.Chrome()
        driver.get('https://www.domondo.fr/volets-roulants/volets-roulants-renovation/volet-roulant-renovation')
        ...
        for index in index_list:
            ...
            for column in column_list: 
                ...
                if float(column * index) > 8000000:
                    break

                profiles = driver.find_element(By.ID, 'cf-step-4326')
                ...
                for ind, profile in enumerate(profiles):
                    ...
                    match profile_name:
                        ...
                        case 'PVC 52 mm':
                            self.df_roller_pvc52.loc[index, column] = product_price
                        case 'Aluminium 39 mm':
                            self.df_roller_alu39.loc[index, column] = product_price
                        case 'Aluminium 52 mm':
                            self.df_roller_alu52.loc[index, column] = product_price
        driver.quit()